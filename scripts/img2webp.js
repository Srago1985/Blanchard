...
function processHtml(filePath) {
  ...
  html = html.replace(regex, (match, before, src, ext, after, offset, fullHtml) => {
    const webp = `${src}.webp`;

    const beforeMatch = fullHtml.slice(0, offset);
    const lastPictureOpen = beforeMatch.lastIndexOf("<picture");
    const lastPictureClose = beforeMatch.lastIndexOf("</picture>");

    // If image is already inside <picture>, add only a missing webp source.
    if (lastPictureOpen > lastPictureClose) {
      return `<source srcset="${webp}" type="image/webp">\n  <img ${before}src="${src}.${ext}"${after}>`;
    }
